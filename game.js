...
powerImage.ready = false;
powerImage.onload = setAssetReady;
powerImage.src = PATH_POWER;  // source image location set in constants.js
...
//Checks to see if youve pressed space creating a bullet!
...
//If you press a key this will happen
...
//when you let go of a key, this will happen
...
{		
    ...
    ctx.fillText("Press Enter to Start", 50,210);
    }
    else{
    if(!gameover){
    ...
    if(right){
        ...
        //drawing pacman same for ghosts!
    ...
    ctx.drawImage(charImage,
        ...
        //moving the image same for ghosts!
	...
	if (pink.IMAGE_START_X >= 2*pink.SPRITE_WIDTH){
       pink.IMAGE_START_X = 0;
         ...
         //POWER UPS   
    if((score % 15) == 0 && score !=0 && Pacman.speed < 5){
        powerUpExists = true;
    }
    if(powerUpExists){
        
        ctx.drawImage(powerImage,146,146);
        
        if (
        (Pacman.CHAR_START_X + 16 > 146 &&
         Pacman.CHAR_START_Y + 16 > 146) &&
        (Pacman.CHAR_START_X < 154 &&
         Pacman.CHAR_START_Y < 154))
            {
                if(Pacman.speed < 5){
                Pacman.speed+= .2;// 5;
                }
